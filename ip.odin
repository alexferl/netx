...
network_range4 :: proc(network: IP4_Network) -> (first: net.IP4_Address, last: net.IP4_Address) {
...
usable_host_range4 :: proc(network: IP4_Network) -> (net.IP4_Address, net.IP4_Address, bool) {
	if network.prefix_len >= 31 {
		return {}, {}, false
	}

	network_addr, broadcast := network_range4(network)
	first, first_ok := next_ip4(network_addr)
	if !first_ok {
		return {}, {}, false
	}

	last, last_ok := prev_ip4(broadcast)
	if !last_ok {
		return {}, {}, false
	}
...
network_range6 :: proc(network: IP6_Network) -> (first: net.IP6_Address, last: net.IP6_Address) {
	first = network.address
...
usable_host_range6 :: proc(network: IP6_Network) -> (net.IP6_Address, net.IP6_Address, bool) {
	if network.prefix_len >= 127 {
		return {}, {}, false
	}

	network_addr, broadcast := network_range6(network)
	first, first_ok := next_ip6(network_addr)
	if !first_ok {
		return {}, {}, false
	}

	last, last_ok := prev_ip6(broadcast)
	if !last_ok {
		return {}, {}, false
	}
